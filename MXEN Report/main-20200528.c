...
////////// MOTOR CONTROL PORT A SETUP //////////
	...
	while(1)
	{
		...
		static int16_t left_sensor  = side_to_mm(adc_read(4)) - SIDE_OFFSET;
		static int16_t front_sensor = front_to_mm(adc_read(5)) - CENTER_OFFSET;
		static int16_t right_sensor = side_to_mm(adc_read(6)) - SIDE_OFFSET;
		...
		{
			static const int16_t SIGNIFICANT_READING_MM = 300;
			static const int16_t SIGNIFICANT_READING_MM_SIDE = 200;
			static const int16_t WALL_DISTANCE = 80;
			...
			switch(current_state)
			{
				...
				case FOLLOW_LEFT:
						...
						if(left_sensor < WALL_DISTANCE)
							right_speed /= (WALL_DISTANCE - left_sensor);
						else if(left_sensor > WALL_DISTANCE + TOLERANCE)
							left_speed /= (left_sensor - WALL_DISTANCE - TOLERANCE);
					break;
				case FOLLOW_RIGHT:
						left_speed  = 512;
						right_speed = 512;

						if(right_sensor < WALL_DISTANCE)
							left_speed /= (WALL_DISTANCE - right_sensor);
						else if(right_sensor > WALL_DISTANCE + TOLERANCE)
							right_speed /= (right_sensor - WALL_DISTANCE - TOLERANCE);
					break;
				case APPROACH_FRONT:
						if(front_sensor < WALL_DISTANCE)
						{
							left_speed /= WALL_DISTANCE - front_sensor;
							right_speed /= WALL_DISTANCE - front_sensor;
						}
						else
						{
							left_speed  = 512;
					...
					break;
				case CAUTION_FRONT_LEFT:
						if(front_sensor < WALL_DISTANCE  || left_sensor < WALL_DISTANCE)
						{
							left_speed /= front_sensor < left_sensor ? WALL_DISTANCE - front_sensor : WALL_DISTANCE - left_sensor;
						}
						else
						{
							//left_speed /= front_sensor < left_sensor ? front_sensor - WALL_DISTANCE : left_sensor - WALL_DISTANCE;
							left_speed = 512;
						}
						right_speed /= left_speed / 2;
					break;
				case CAUTION_FRONT_RIGHT:
						if(front_sensor < WALL_DISTANCE || right_sensor < WALL_DISTANCE)
						{
							right_speed /= front_sensor < right_sensor ? WALL_DISTANCE - front_sensor : WALL_DISTANCE - right_sensor;
						}
						else
						{
							//right_speed /= front_sensor < right_sensor ? front_sensor - WALL_DISTANCE : right_sensor - WALL_DISTANCE;
							right_speed = 512;
						}
				...
				case CAUTION_LEFT_RIGHT:
						...
						{
							left_speed /= left_sensor - right_sensor;
							right_speed = 512;
						}
						else if (right_sensor > left_sensor + TOLERANCE)
						{
							right_speed /= right_sensor - left_sensor;
							right_speed = 512;
						}
						else
						{
