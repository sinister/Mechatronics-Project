...
char buffer[64]; // Large buffer that can be used for formatting strings before writing to serial.
...
{
	// 'points' is a circular buffer to keep track of the last 'point_count' readings.
	// When a value is removed from the buffer, it is subtracted from the sum. When
	// it is added to the buffer it is added to the sum. This means that we don't need
	// to sum the whole array each time.
